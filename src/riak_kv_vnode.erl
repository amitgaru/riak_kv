...
put_merge(false, false, CurObj, UpdObj, {NewEpoch, VId}, _StartTime) -> % coord=false, LWW=false
    ...
    ?LOG_INFO("riak_kv_vnode:put_merge/6 called put args coord=false, LWW=~p, CurObj=~p, UpdObj=~p, _NewEpoch=~p, VId=~p, StartTime=~p", [LWW, CurObj, UpdObj, _NewEpoch, VId, StartTime]),
